...
def test_docker_helper_scripts_hide_compose_profile_details():
    ...
    assert "docker compose --profile app run --rm musicagent-cli" in docker_cli
    assert "docker compose --profile app run --rm musicagent-cli create" in docker_create
    ...
    assert "docker compose --profile local-llm run --rm ollama-pull" in docker_local_llm_pull
    assert (
        "docker compose --profile local-llm run --rm musicagent-local-llm-cli create"
        in docker_local_llm_create
    )
    assert "docker compose --profile local-llm up musicagent-local-llm-web" in docker_local_llm_web
    assert (
        "docker compose --profile local-llm run --rm musicagent-local-llm-cli llm-check"
